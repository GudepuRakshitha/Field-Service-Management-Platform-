    ...
    <div className="min-h-screen flex bg-[#061121]">
      ...
      >
        ...
        <div className="p-3 m-3 space-y-2 shrink-0">
          ...
          <button
            onClick={toggleTheme}
            className="w-full flex items-center justify-between px-3.5 py-2.5 rounded-xl border border-amber-500/25 bg-amber-950/30 hover:bg-amber-900/50 transition-all group"
            title={`Switch to ${theme === 'dark' ? 'Light' : theme === 'light' ? 'Comic' : 'Dark'} Mode`}
          >
            <div className="flex items-center gap-2.5">
              {theme === 'dark' ? (
            ...
            <div className={`w-8 h-4 rounded-full relative transition-colors duration-300 ${theme === 'dark' ? 'bg-amber-900/60' : theme === 'light' ? 'bg-sky-400/40' : 'bg-yellow-600/40'}`}>
          ...
          {/* User Card */}
          <div className="p-3 rounded-2xl bg-amber-950/30 border border-amber-500/25">
